...
const Emoji = ({ e, color, size = 20 }: { e: string; color?: string; size?: number }) => (
...
type Cat  = 'all' | 'design' | 'dev' | 'pm' | 'data' | 'ops' | 'qa' | 'mkt' | 'sales'
...
export default function TabBarUsage() {
  ...
  return (
    <SafeAreaView style={u.safe}>
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={u.scroll}>
...
const Section = ({ label, children }: { label: string; children: React.ReactNode }) => (
  <View style={u.section}>
    <Text style={u.sectionLabel}>{label}</Text>
    {children}
  </View>
)
