...
const Thumb = ({
  ...
  const latestTrackWidth = (0, _react.useRef)(trackWidth);
  const latestOnMove = (0, _react.useRef)(onMove);
  const latestOnEnd = (0, _react.useRef)(onEnd);
  const latestOnStart = (0, _react.useRef)(onStart);
  const startFractionRef = (0, _react.useRef)(position);
  (0, _react.useEffect)(() => {
  ...
  const panResponder = (0, _react.useRef)(_reactNative.PanResponder.create({
    ...
    onPanResponderMove: (_, gs) => {
      ...
      const newFraction = clamp(startFractionRef.current + gs.dx / width, 0, 1);
...
const StyledSlider = ({
  ...
  const isDraggingLow = (0, _react.useRef)(false);
  const isDraggingHigh = (0, _react.useRef)(false);
  ...
  const prevHighRef = (0, _react.useRef)(undefined);
  (0, _react.useEffect)(() => {
  ...
  // ─── Low thumb handlers ───────────────────────────────────────────────────
  const handleLowStart = (0, _react.useCallback)(() => {
  ...
  const handleHighStart = (0, _react.useCallback)(() => {
